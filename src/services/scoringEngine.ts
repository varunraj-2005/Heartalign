...
export function calculateQuestionScore(
  ...
  val1: string | number,
  val2: string | number
): { score: number | null; conflictFlag?: ConflictFlag } {
  ...
  if (question.question_type === 'multiple_choice_match') {
    ...
    let conflictFlag: ConflictFlag | undefined;
    ...
    if (question.scoring_rules?.complementary_conflict_pairs) {
      for (const pair of question.scoring_rules.complementary_conflict_pairs) {
        if (
          (String(pair.val1) === str1 && String(pair.val2) === str2) ||
...
export function generateCategoryInsight(
  category: Category,
  score: number,
  lowestScoringQuestion?: { prompt: string; p1Val: string; p2Val: string }
): string {
  if (score >= 85) {
    switch (category) {
...
export function computeCompatibilityScore(
  ...
  const reflections: SideBySideReflection[] = [];
  ...
  const categoryBreakdown: Partial<Record<Category, CategoryScoreDetail>> = {};
  let weightedSum = 0;
  ...
  return {
    ...
    category_breakdown: categoryBreakdown as Record<Category, CategoryScoreDetail>,
