...
router.post('/sessions', (req, res) => {
    try {
        ...
        const couple_id = existing_couple_id || `cpl_${(0, uuid_1.v4)().substring(0, 8)}`;
        (0, database_1.dbCreateCouple)(couple_id);
        const session_id = `ses_${(0, uuid_1.v4)()}`;
        const partner1_id = `ptr_${(0, uuid_1.v4)().substring(0, 8)}`;
        ...
        (0, database_1.dbCreateSession)(newSession);
...
router.post('/sessions/join', (req, res) => {
    try {
        ...
        const session = (0, database_1.dbGetSessionByCode)(invite_code);
        if (!session) {
        ...
        const partner2_id = `ptr_${(0, uuid_1.v4)().substring(0, 8)}`;
        session.partner2_id = partner2_id;
        session.partner2_name = partner2_name.trim();
        session.status = 'in_progress';
        (0, database_1.dbUpdateSession)(session);
        res.json({
            success: true,
...
router.get('/sessions/:id/status', (req, res) => {
    try {
        ...
        if (session.partner2_id) {
            const p2Answers = (0, database_1.dbGetPartnerAnswers)(session.id, session.partner2_id);
...
router.post('/sessions/:id/answers', (req, res) => {
    try {
        ...
        (0, database_1.dbSaveAnswers)(session.id, partner_id, answers);
        // Check if both partners have finished now
        const p1Answers = (0, database_1.dbGetPartnerAnswers)(session.id, session.partner1_id);
        let p2Answers = {};
        if (session.partner2_id) {
            p2Answers = (0, database_1.dbGetPartnerAnswers)(session.id, session.partner2_id);
        }
        ...
        if (bothCompleted) {
            ...
            const scoreResult = (0, scoringEngine_1.computeCompatibilityScore)(questions, p1Answers, p2Answers, session.id, session.couple_id, session.partner1_name, session.partner2_name || 'Partner 2');
            (0, database_1.dbSaveScore)(scoreResult);
        }
        res.json({
            success: true,
    ...
    try {
        ...
        if (!scoreResult) {
            ...
            if (Object.keys(p1Answers).length > 0 && Object.keys(p2Answers).length > 0) {
                const questions = (0, database_1.dbGetQuestions)();
                scoreResult = (0, scoringEngine_1.computeCompatibilityScore)(questions, p1Answers, p2Answers, session.id, session.couple_id, session.partner1_name, session.partner2_name || 'Partner 2');
                (0, database_1.dbSaveScore)(scoreResult);
                session.status = 'completed';
                session.completed_at = new Date().toISOString();
                (0, database_1.dbUpdateSession)(session);
